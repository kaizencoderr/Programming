...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        range_count = 0
        while location.is_hive==False:
            if location.bees!=[] and self.min_range <= range_count <= self.max_range:
                return choose_bee(location.bees)
            else:
                location = location.entrance
                range_count += 1
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        bees_in_location = list(self.place.bees)
        if amount < self.health:
            for bees in bees_in_location:
                bees.reduce_health(amount)
        elif amount >= self.health:
            for bees in bees_in_location:
...
class HungryAnt(Ant):
    ...
    def __init__(self, health=1, chew_countdown=0):
        """Create a HungryAnt with a health quantity and a chew countdown."""
        super().__init__(health)
        self.chew_countdown = chew_countdown
    
    def action(self, gamestate):
        """First check if HungryAnt is chewing; if so, decrement its chew_countdown. 
        ...
        if self.chew_countdown > 0:
            ...
            random_bee = choose_bee(beez)
            Bee.reduce_health(random_bee, random_bee.health)
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        """Deal 1 damage to bee and contained bee does action."""
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        if insect.is_waterproof == False:
            return Insect.reduce_health(insect, insect.health)
        else:
            return
        # END Problem 10
