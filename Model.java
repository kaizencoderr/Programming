...
class Model {
    ...
    void forNeighbors(Place place, Consumer<Place> action) {
        if (place.row > 0) {
            Place above = Place.pl(place.row - 1, place.col);
            action.accept(above);
        }
        if (place.row + 1 < _height) {
            Place below = Place.pl(place.row + 1, place.col);
            action.accept(below);
        }
        if (place.col > 0) {
    ...
    final boolean solved() {
        int n = 0;
        for (int i = 0; i < this.colorsPresent().length; i++) {
            if (this.colorsPresent()[i]) {
                n++;
            }
        }
        return (n == 1);
    ...
    boolean roundOver() {
        return (this.solved() || (this._limit == _current));
    }
    ...
    HashSet<Place> findRegion(Place start, int color, HashSet<Place> result) {
        ...
        result.add(start);
        forNeighbors(start, (ressy) -> {
            findRegion(ressy, color, result);
        });
        return result;
    }
    ...
    void setActiveRegionColor(int newColor) {
        HashSet<Place> newnew = new HashSet<Place>();
        for (Place x : _active) {
            _cells[x.row][x.col] = newColor;
        }
        _active = findRegion(Place.pl(0, 0), newColor, newnew);
        _current++;
    ...
    Set<Place> adjacentCells(int color) {
        HashSet<Place> adjacent = new HashSet<Place>();
        for (Place x : _active) {
            forNeighbors(x, (zz) ->
                findRegion(zz, color, adjacent));
        }
        return adjacent;
    }
